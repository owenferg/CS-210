...
def get_fires_utm(path: str) -> list[tuple[int, int]]:
    ...
    with open(path, newline="", encoding="utf-8") as source_file:
        ...
        result = [] # To be returned, final list of tuple pairs
...
def centroid(points: list[tuple[int, int]]) -> tuple[int, int]:
    ...
    if not points:
        return (0,0) # No division by zero check
    ...
    mean_x = int(x_sum / len(points))
    mean_y = int(y_sum / len(points))
...
def closest_index(point: tuple[int, int], centroids: list[tuple[int, int]]) -> int:
    ...
    1
    """
    # Variables initializing as -1 as starting variable
    closest_index = -1
    closest_point = -1
    index = 0 

    for i in centroids:
        # If first set of points in loop, set of points is closest point/index
        if closest_index == -1 or closest_point == -1:
            closest_index = index
            closest_point = i
        # If set of points is closer than previous closest set of points, replace variable respectively
        elif sq_dist(point, i) < sq_dist(point, closest_point):
            closest_index = index
            closest_point = i
        index += 1 # Keep track of index by accumulating at end of loop
...
def move_points(fire_map: graphics.utm_plot.Map,
                points:  list[tuple[int, int]], 
                symbols: list): 
    if len(symbols) != len(points): # Error check
        return
